#!/usr/bin/env python3
"""
Actualiza las funciones crearGraficaHumedad y crearGraficaAceite para usar nueva estructura de rangos
"""

with open('static/js/custom/graficas_base_frita.js', 'r', encoding='utf-8') as f:
    content = f.read()

# Actualizar crearGraficaHumedad - parte de datasets
old_datasets_humedad = '''        // Obtener rangos del producto seleccionado
...
new_datasets_humedad = '''        // Obtener rangos del producto seleccionado (usa RANGOS_FISICOQUIMICOS_FINAL)
...
content = content.replace(old_datasets_humedad, new_datasets_humedad)

# Actualizar cálculo eje Y para humedad
old_calc_y_humedad = '''        // Calcular rango dinámico del eje Y - AUTO-AJUSTE COMPLETO
...
new_calc_y_humedad = '''        // Calcular rango dinámico del eje Y usando la función calcularEjeY
        const { yMin, yMax } = calcularEjeY(datos, rangos);
        console.log('📏 Humedad - Eje Y auto-ajustado:', yMin.toFixed(2), 'a', yMax.toFixed(2));'''

content = content.replace(old_calc_y_humedad, new_calc_y_humedad)

with open('static/js/custom/graficas_base_frita.js', 'w', encoding='utf-8') as f:
    f.write(content)

print("✅ crearGraficaHumedad actualizada")
